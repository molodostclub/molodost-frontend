					...
					<form onSubmit={onSubmit}>
						...
						<div>
							...
							{apartments2.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="apartments2" item={item} required={false} onChange={() => onChangeCheckbox(item.value, apartments2, (data) => setApartments2(data))} />
								</div>
							))}
							<br />
							...
							<small className={styles.checkBoxNote}>
								Предупреждаем, что в&nbsp;итоге вариант платить за&nbsp;еду на&nbsp;месте получится дороже, а&nbsp;вокруг не&nbsp;так много вкусных мест, хотя недавно у&nbsp;нас на&nbsp;Алтае появился прекрасный ресторан &laquo;Вкусно и&nbsp;точка&raquo;.
							</small>
							{/* Allergiya */}
							<br />
							<div className={indent.mt_4}>
							...
							{wantTry.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="wantTry" item={item} required={false} onChange={() => onChangeCheckbox(item.value, wantTry, (data) => setWantTry(data))} />
								</div>
							))}
							<br />
							<br />
							<small className={styles.noteBlack}>У&nbsp;нас большой выбор напитков из&nbsp;коллекции нашего постоянно партнера Simple, но&nbsp;если для вас принципиально что-то особенное, то&nbsp;давайте это обсудим.</small>
							<br />
							{/* Alko */}
							{alko.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="alko" item={item} required={false} onChange={() => onChangeCheckbox(item.value, alko, (data) => setAlko(data))} />
								</div>
							))}
							<br />
							{/* Coffee */}
							...
							{coffee.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="coffee" item={item} required={false} onChange={() => onChangeCheckbox(item.value, coffee, (data) => setCoffee(data))} />
								</div>
							))}
						</div>
						...
						<div>
							...
							{restTime.map((item, i) =>
								i === restTime.length - 1 ? (
									<>
										...
										<div key={i} className={indent.mt_2}>
											<BaseCheckbox name="restTime" item={item} required={false} onChange={() => onChangeCheckbox(item.value, restTime, (data) => setRestTime(data))} />
										</div>
									</>
								) : (
									<div key={i} className={indent.mt_2}>
							...
							{blagodat.map((item, i) =>
								i === blagodat.length - 1 ? (
									<>
										...
										<div key={i} className={indent.mt_2}>
											<BaseCheckbox name="blagodat" item={item} onChange={() => onChangeCheckbox(item.value, blagodat, (data) => setBlagodat(data))} />
										</div>
									</>
								) : (
									<div key={i} className={indent.mt_2}>
							...
							{/* bud sponsorom */}
							<div className={indent.mt_4}>
								<Label caption="Мы&nbsp;постоянно проводим на&nbsp;базе разные культурные события. Если вам важно что-то конкретное в&nbsp;ваши даты, вы&nbsp;можете стать спонсором нашего культурного вечера, это стоит 45&nbsp;000&nbsp;₽:" />
							</div>
							{sponsor.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="sponsor" item={item} onChange={() => onChangeCheckbox(item.value, sponsor, (data) => setSponsor(data))} />
								</div>
							))}
							<div className={indent.mt_4}>
						...
						<div>
							...
							{plany.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="plany" item={item} onChange={() => onChangeCheckbox(item.value, plany, (data) => setPlany(data))} />
								</div>
							))}
							<div className={indent.mt_4}>
								<small className={styles.note}>Эти услуги оказывают наши партнеры и&nbsp;нам нужно договариваться заранее, все остальное у&nbsp;нас свое.</small>
							</div>
							<br />
							{/* Na Mars */}
							<div className={indent.mt_4}>
							...
							{naMars.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="naMars" item={item} onChange={() => onChangeCheckbox(item.value, naMars, (data) => setNaMars(data))} />
								</div>
							))}
							<br />
							...
							{travelPriority.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="travelPriority" item={item} onChange={() => onChangeCheckbox(item.value, travelPriority, (data) => setPriority(data))} />
								</div>
							))}
							<br />
							...
							{wishes.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="wishes" item={item} onChange={() => onChangeCheckbox(item.value, wishes, (data) => setWishes(data))} />
								</div>
							))}
							<div className={indent.mt_4}>
							...
							{forChildren.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="forChildren" item={item} onChange={() => onChangeCheckbox(item.value, forChildren, (data) => setForChildren(data))} />
								</div>
							))}
							<br />
							...
							{stepnik.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="stepnik" item={item} onChange={() => onChangeCheckbox(item.value, stepnik, (data) => setStepnik(data))} />
								</div>
							))}
							<br />
							{/* Ne ezdit nikuda */}
							...
							{neEzdit.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="neEzdit" item={item} onChange={() => onChangeCheckbox(item.value, neEzdit, (data) => setNeEzdit(data))} />
								</div>
							))}
						</div>
						...
						<div>
							...
							{nanny.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="Nanny" item={item} onChange={() => onChangeCheckbox(item.value, nanny, (data) => setNanny(data))} />
								</div>
							))}
							<br />
							{/* Airport */}
							...
							{airport.map((item, i) => (
								<div key={i} className={indent.mt_2}>
									<BaseCheckbox name="airport" item={item} onChange={() => onChangeCheckbox(item.value, airport, (data) => setAirport(data))} />
								</div>
							))}
							<br />
							{/*<small className={cn(indent.mt_2, styles.note)}>
