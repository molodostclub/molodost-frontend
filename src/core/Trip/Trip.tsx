...
export const Trip: FC<{ trip: TripModel }> = ({
    ...
    <div className={styles.content}>
      ...
      <div className={styles.grid}>
        <div className={styles.pricing}>
          <PriceTableRow
            title={trip.isAllDay ? "Весь день" : trip.durationText}
          >
            {trip.priceAdult && <div className={priceTable.cell}>
              <div>Взрослый</div>
              <div className={priceTable.accent}>{formatPriceWithSign(trip.priceAdult)}</div>
            </div>}
            {trip.priceChild && <div className={priceTable.cell}>
              <div>Ребенок</div>
        ...
        {!!trip.notes.length && <ul className={styles.attentionList}>
          {trip.notes.map(
            (item) => {
              return (
      ...
      <div className={styles.backLink}>
        <BrandLink linkTo="/kak-my-puteshevstvuem" direction="left">Все путешествия</BrandLink>
      </div>
    </div>
  );
};
